...
const CalendarPage: React.FC<CalendarPageProps> = ({ showToast }) => {
  ...
  const loadData = async () => {
    ...
    setIsLoading(true);
    try {
      const [aptData, profData, roomData, configData] = await Promise.all([
  ...
  const timeSlots = useMemo(() => {
    const startStr = config.horario_abertura || '08:00';
    const endStr = config.horario_fechamento || '20:00';
    const step = parseInt(config.duracao_sessao_minutos || '60');
    ...
    return slots;
  }, [config.horario_abertura, config.horario_fechamento, config.duracao_sessao_minutos]);
  ...
  const handleOpenNew = () => {
    ...
    setFormData({
      ...
      room_id: rooms[0]?.id || '',
      date: dateStr,
      time: config.horario_abertura || '09:00',
      duration: config.duracao_sessao_minutos || '60',
      status: 'confirmed'
    });
    setShowModal(true);
  };
  ...
  const handleDeleteAppointment = async () => {
    if (!editingId) return;
    if (!confirm('ATENÇÃO: Isso excluirá PERMANENTEMENTE o agendamento do banco de dados e do Google Agenda. Deseja continuar?')) return;
    
    setIsSaving(true);
    try {
        // Primeiro deletamos do Google Calendar enquanto o registro ainda existe no banco
        await supabase.functions.invoke('google-calendar-sync', {
            body: { appointmentId: editingId, action: 'DELETE' }
        });

        // Agora deletamos do banco de dados
        const { error } = await supabase.from('agendamentos').delete().eq('id', editingId);
        if (error) throw error;
              ...
              <div key={col.id} className="flex-1 border-r border-slate-100 last:border-r-0 relative">
                ...
                {filteredAppointments
                  ...
                  .map(ap => {
                    ...
                    const apStartMinutes = start.getHours() * 60 + start.getMinutes();
                    const top = ((apStartMinutes - startMinutes) / step) * 64;

                    const durationMin = (end.getTime() - start.getTime()) / 60000;
                    const height = (durationMin / step) * 64;
                    ...
                    return (
                      <div 
                        ...
                        className={`absolute left-1 right-1 rounded-xl p-3 shadow-sm border ${ap.status === 'confirmed' ? `${profColor.bg} ${profColor.text} ${profColor.border}` : `${STATUS_CONFIG[ap.status]?.bg} ${STATUS_CONFIG[ap.status]?.color} ${STATUS_CONFIG[ap.status]?.ring}`} z-10 transition-all hover:scale-[1.02] cursor-pointer overflow-hidden`}
  ...
  return (
    <div className="p-8 flex flex-col bg-slate-50/30 min-h-screen">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 space-y-4">
                  ...
                  <div>
                    ...
                    <input required type="number" step={config.duracao_sessao_minutos || "30"} value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full px-5 py-3 rounded-xl bg-slate-50 border border-slate-100 outline-none focus:border-indigo-500 font-bold" />
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-3 pt-4">
                {editingId ? (
                   <>
                    ...
                    <button type="button" onClick={handleDeleteAppointment} disabled={isSaving} className="w-full px-6 py-3 rounded-2xl font-black uppercase tracking-[0.2em] text-[10px] text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-all flex items-center justify-center gap-2 border border-dashed border-slate-200 hover:border-rose-200">
